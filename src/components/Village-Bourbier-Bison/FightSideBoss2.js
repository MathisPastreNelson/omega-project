...
function FightSideBoss2(component) {
    ...
    const attackOne = () => {
        ...
        if (enemyHp - getRandomNumber1() < 1) {
            ...
            localStorage.setItem("SideQuest05", "ok")
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            ...
            let newDamage = Math.floor(Math.random() * (7 + 14) * damageReductionCoefficient);
            setDamage(newDamage);
